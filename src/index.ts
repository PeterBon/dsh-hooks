...
export function apply(ctx: Context, config: Config = {}) {
   ...
   */
  const trackUsageDay = (origin: HookContext, sessionId: string): void => {
    ...
    if (finished !== undefined) runMatching(usageDailyContext(origin, finished.totals))
  }
  ...
  ctx.on('session/event', (session: Session, event: unknown) => {
    ...
    // new one.
    if (hasUsageDailyHooks) trackUsageDay(classified, sessionId)

...
export function parsePatchText(text: string): unknown[] {
  let entries: unknown
  try {
 ...
 */
export function validateHookWire(hooks: HookWireSpec[]): string | null {
  for (const [i, hook] of hooks.entries()) {
    const label = `hook #${i + 1}`
    if (!HOOK_EVENTS.includes(hook.on as (typeof HOOK_EVENTS)[number])) {
      return `${label}：无效事件 ${hook.on}`
    }
    if (hook.when !== undefined && !TURN_END_REASONS.includes(hook.when as (typeof TURN_END_REASONS)[number])) {
      return `${label}：无效 when 原因 ${hook.when}`
    }
    if (hook.match !== undefined) {
      for (const [field, pattern] of Object.entries(hook.match)) {
        if (field === '') return `${label}：match 字段名不能为空`
        try {
          new RegExp(pattern)
        } catch (error) {
 ...
 */
export function patchTextWithHooks(existingText: string, hooks: HookWireSpec[]): string {
  const entries = parsePatchText(existingText)
  let found = false
  for (const entry of entries) {
    if (entry !== null && typeof entry === 'object' && (entry as { id?: unknown }).id === 'dsh-hooks') {
      const target = entry as { name?: unknown; config?: Record<string, unknown> }
...
export function backupPathFor(patchFile: string, now: Date = new Date()): string {
 ...
 */
export function writeHooksConfig(patchFile: string, hooks: HookWireSpec[]): WriteHooksResult {

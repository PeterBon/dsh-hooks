...
export function writeConfig(
  configPath: string,
  { appId, appSecret, targetType = 'open_id', targetId, resultMaxChars = 300 }: {
    appId: string
    appSecret: string
    targetType?: string
    targetId: string
...
export function mergePatchYaml(existingText: string, { scriptPath }: { scriptPath: string }): string {
  ...
  for (const entry of entries) {
    if (entry && typeof entry === 'object' && (entry as { id?: unknown }).id === 'dsh-hooks') {
      ;(entry as { name?: unknown }).name = 'dsh-hooks'
      ;(entry as { config?: unknown }).config = { hooks }
      found = true
      break
    }
  }
 ...
 */
export async function runFeishuSetup(options: RunFeishuSetupOptions = {}): Promise<FeishuSetupResult> {
  ...
  const registerAppFn =
    options.registerAppFn ?? (await import('@larksuiteoapi/node-sdk')).registerApp
...
export async function runFeishuTest(options: RunFeishuTestOptions = {}): Promise<string> {
  const print = options.print ?? console.log
  ...
  const { app_id: appId, app_secret: appSecret, target_id: targetId } = file as {
    app_id?: unknown
    app_secret?: unknown
    target_id?: unknown
  }
 ...
 */
export function readFeishuSummary(configPath: string = FEISHU_CONFIG_PATH): FeishuSummary {
  const empty: FeishuSummary = {
    configured: false,
    appId: null,
    targetKind: null,
    target: null,
 ...
 */
export function updateFeishuResultMaxChars(configPath: string = FEISHU_CONFIG_PATH, value: number): number {
  ...
  if (!existsSync(configPath)) throw new Error('尚未连接飞书，请先扫码连接')
  let file: unknown
  try {
    file = JSON.parse(readFileSync(configPath, 'utf8'))
  } catch {
  ...
  if (typeof file !== 'object' || file === null) throw new Error('飞书配置文件解析失败，请重新连接')
  ;(file as { result_max_chars?: unknown }).result_max_chars = rounded

  ...
  hooks: Schema.array(
    Schema.object({
      on: Schema.union([...HOOK_EVENTS]).description(
        '触发事件：turn/start | turn/end | step/end | tool/call | tool/result | user/message | approval/asked | approval/decided | session/title | session/created | session/disposed | agent/created | agent/disposed | agent/error | agent/status',
      ),
